...
### 9) exe 파일만 더블클릭하면 실행되는 프로그램인가요?

**지금 문서 기준으로는 아니에요.**  
현재는 개발자 실행 방식(서버 + 워커 실행)이라서, 일반 앱처럼 `something.exe` 하나만 더블클릭해서 끝나는 형태가 아닙니다.

쉽게 비유하면:
- exe 앱: 버튼만 누르면 바로 켜짐
- 현재 방식: 엔진(백엔드) + 작업기사(워커)를 각각 켜야 함

즉, 지금은 아래 2개를 직접 실행해야 합니다.
1. `uvicorn app.main:app --reload` (서버)
2. `celery -A app.worker worker -l info` (워커)

원하면 나중에 아래처럼 **exe 형태로 패키징**할 수는 있습니다.
- 백엔드/워커를 서비스로 상시 실행
- 사용자용 데스크톱 앱(Electron/Tauri 등)에서 버튼 한 번으로 호출
- 또는 설치 프로그램(.exe)로 묶어서 “실행→업로드→결과 확인” 흐름 제공
